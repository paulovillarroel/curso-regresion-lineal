...
Este documento cubre el diagnóstico de los **5 supuestos clásicos** de la regresión lineal por mínimos cuadrados ordinarios (OLS), más el análisis de **observaciones influyentes** y una discusión sobre **endogeneidad**:
...
6. **Outliers, leverage e influencia** — observaciones que pueden distorsionar los resultados.

Para cada supuesto se presenta el test formal, el diagnóstico visual, las consideraciones prácticas y las referencias bibliográficas correspondientes. El documento incluye además ejemplos aplicados en contextos clínicos y organizacionales, y funciones reutilizables que consolidan todos los diagnósticos.
...
Además, los tests de supuestos sirven para **evaluar** un modelo ya definido, no para decidir qué modelo usar. Un test que rechaza no significa automáticamente que debas cambiar tu especificación — la decisión de qué variables incluir debe basarse en el conocimiento del problema y el diseño de investigación, no en p-values de tests diagnósticos.
:::

::: {.callout-important}
## Antes de diagnosticar: ¿el modelo incluye las variables correctas?

Este curso se enfoca en una pregunta específica: **dado un modelo ya especificado, ¿puedo confiar en sus resultados estadísticos?** Pero hay una pregunta previa igual de importante: **¿estoy incluyendo las variables correctas en el modelo?**

Si omites una variable relevante que está correlacionada con tus predictores (un *confusor*), o incluyes una variable que no deberías (un *mediador* en el camino causal), los coeficientes estarán sesgados — y ningún test de supuestos lo detectará. Los residuos pueden verse perfectos mientras los coeficientes cuentan una historia equivocada.

Pensándolo con una analogía: diagnosticar supuestos es como revisar que el motor de un auto funciona bien. Pero si te subiste al auto equivocado, de nada sirve que el motor esté perfecto. La identificación causal — decidir qué variables incluir y por qué — es elegir el auto correcto.

Para profundizar en identificación causal, dos recursos complementarios:

- @huntingtonklein2021 — Cubre diagramas causales (DAGs), back-door criterion y cómo decidir qué variables incluir en un modelo [Cap. 5–8]. Enfoque conceptual con ejemplos en R, Stata y Python.
- @rcausal2024 — Guía práctica de inferencia causal **implementada en R**. Cubre DAGs con `ggdag` y `dagitty` [Cap. 4], propensity scores [Cap. 8–9], datos faltantes y error de medición [Cap. 15], y análisis de sensibilidad [Cap. 16]. Especialmente útil si trabajas en R y quieres pasar de los diagnósticos de este curso a la estimación de efectos causales.
:::

### Referencias generales

@fox2016; @wooldridge2020; @faraway2015; @taubes2010^[Clifford Henry Taubes, Departamento de Matemáticas de Harvard. No confundir con Gary Taubes, autor de libros sobre nutrición. Estas son notas de clase de probabilidad, estadística y álgebra lineal.].
...
::: {.callout-important}
## Los supuestos no son para que el modelo "funcione" — son para que la inferencia sea válida

Esta distinción es crucial. Un modelo con supuestos violados **sigue haciendo predicciones** — y esas predicciones pueden incluso ser razonables para interpolar dentro del rango de los datos. Lo que se rompe es la **inferencia**: los p-values, los intervalos de confianza y las conclusiones sobre si un efecto es "significativo" o no.

| Objetivo del modelo | ¿Se necesitan los supuestos? |
|---|---|
| **Predicción** (¿cuánto va a costar esta hospitalización?) | Menos crítico. Si el modelo predice bien en datos nuevos, los supuestos importan menos. |
| **Inferencia** (¿el tratamiento reduce la estadía?) | **Crítico.** Sin supuestos, los p-values y los IC pueden ser completamente inválidos — podrías declarar significativo un efecto que no existe, o viceversa. |

Si tu objetivo es solo predecir, herramientas como machine learning pueden ser más apropiadas (y no requieren estos supuestos). Pero si necesitas **entender y comunicar relaciones causales o asociativas** — que es lo que hacen la mayoría de los análisis en salud, epidemiología y gestión — la inferencia es lo central, y los supuestos son innegociables.
:::
...
- **Sesgo por variable omitida (OVB):** Si una variable relevante se omite del modelo y está correlacionada con los predictores incluidos, el término de error absorbe su efecto. Los estimadores resultantes son **sesgados e inconsistentes**. La magnitud del sesgo es: $\text{sesgo}(\hat{\beta}_1) = \beta_2 \cdot \delta$, donde $\beta_2$ es el efecto de la variable omitida y $\delta$ la relación entre la omitida y el predictor incluido. **Ejemplo automotriz:** si queremos medir el efecto del peso (`weight`) en el rendimiento (`mpg`) pero omitimos la cilindrada del motor, parte del efecto de la cilindrada se "cuela" en el coeficiente de peso, distorsionándolo. **Ejemplo clínico:** supongamos que queremos estimar el efecto de la edad sobre la mortalidad hospitalaria, pero omitimos la presencia de diabetes (una comorbilidad frecuente que se correlaciona con la edad). El modelo atribuirá a la edad parte del efecto que en realidad corresponde a la diabetes — sobreestimando el riesgo asociado a envejecer y subestimando el de la comorbilidad omitida. En gestión de salud, esto puede llevar a asignar recursos basándose en edad cuando el factor modificable es el manejo de la diabetes.
...
| **Normalidad** | Generalmente ignorable con $n$ grande | Transformar $Y$ (Box-Cox) o usar bootstrap para la inferencia (ver ejemplo abajo) |
...
::: {.callout-note}
## Bootstrap: intervalos de confianza sin asumir normalidad

Cuando la normalidad de los residuos falla y la muestra es pequeña (donde el TCL no alcanza a compensar), el **bootstrap** ofrece una alternativa: re-muestrea los datos miles de veces, reajusta el modelo en cada muestra, y construye intervalos de confianza directamente a partir de la distribución empírica de los coeficientes — sin asumir nada sobre la distribución de los errores.

```r
# Bootstrap con car::Boot (requiere install.packages("car"))
library(car)
boot_result <- Boot(modelo, R = 2000)  # 2000 remuestreos
summary(boot_result)     # coeficientes bootstrap
confint(boot_result)     # IC al 95% (sin supuesto de normalidad)
hist(boot_result)        # distribución empírica de cada coeficiente
```

Si los intervalos bootstrap difieren sustancialmente de los intervalos OLS estándar (`confint(modelo)`), la no-normalidad está afectando la inferencia y es mejor reportar los intervalos bootstrap.
:::
...
::: {.callout-tip}
## Regla práctica
- Con $n \geq 30$, la aproximación normal suele ser razonable para distribuciones unimodales y simétricas.
- Con distribuciones muy asimétricas o con colas pesadas, puede requerirse $n \geq 50$, $100$ o más. **Esto es especialmente relevante en datos de salud**: costos hospitalarios, días de estadía, tiempos de espera y conteos de eventos (reingresos, infecciones) casi siempre tienen distribuciones con sesgo positivo fuerte. Para estas variables, $n = 30$ no es suficiente para que el TCL "arregle" el problema — y confiar en la regla de $n \geq 30$ sin verificar puede dar falsa seguridad.
- El TCL **no** aplica si la varianza de los errores es infinita (distribuciones de colas extremadamente pesadas como la Cauchy).
- **En la duda:** compara los intervalos de confianza OLS con intervalos bootstrap. Si difieren sustancialmente, la aproximación normal aún no es adecuada para tu $n$.
:::
...
::: {.callout-warning}
## Antes de diagnosticar: ¿cuántos datos estás perdiendo?

Cuando ejecutas `lm(Y ~ X1 + X2)`, R aplica por defecto `na.action = na.omit`: **elimina silenciosamente toda fila que tenga al menos un `NA` en cualquiera de las variables del modelo**. No lanza error ni advertencia — simplemente reduce el $n$ sin avisarte.

Si los datos faltantes son aleatorios (*Missing Completely at Random*, MCAR), esta eliminación solo reduce potencia. Pero en datos de salud pública, **la falta de datos rara vez es aleatoria**. Si una base de registros oncológicos tiene un 20% de valores faltantes en la variable "estadificación al diagnóstico" y los hospitales que menos reportan son los rurales o los de menor complejidad, el modelo se ajustará solo sobre la población urbana/de alta complejidad. Los coeficientes, los errores estándar y los p-values serán estadísticamente válidos *para esa subpoblación* — pero no representarán a la población real.

**Recomendación práctica:** Antes de correr cualquier modelo, ejecuta:

```r
# ¿Cuántas filas pierde tu modelo?
n_completos <- sum(complete.cases(datos[, c("Y", "X1", "X2")]))
n_total <- nrow(datos)
cat("Observaciones completas:", n_completos, "de", n_total,
    paste0("(", round(100 * (1 - n_completos / n_total), 1), "% perdido)\n"))

# ¿Qué variable tiene más NAs?
colSums(is.na(datos[, c("Y", "X1", "X2")]))
```

Si pierdes más del 5–10% de los datos, investiga **por qué** faltan antes de continuar. La calidad del registro institucional condiciona todo lo que sigue: ningún test de supuestos puede corregir un sesgo de selección introducido por datos faltantes no aleatorios. Para un tratamiento formal de datos faltantes y error de medición en el contexto de inferencia causal, ver @rcausal2024 [Cap. 15].
:::
...
Pero antes de entrar en esta secuencia, recuerda que existe un **paso previo** que este curso no cubre: la **especificación causal** del modelo — es decir, decidir qué variables incluir basándote en el conocimiento del fenómeno que estudias. ¿Es una variable un confusor que debes controlar? ¿Es un mediador que no deberías incluir? Si este paso se hizo mal, el modelo tendrá endogeneidad (ver @sec-por-que) y ningún diagnóstico de residuos lo revelará. Para guías sobre cómo tomar estas decisiones, ver @huntingtonklein2021 [Cap. 5–8] y @rcausal2024 [Cap. 4–6].

Asumiendo que la especificación causal es razonable, el orden de diagnóstico estadístico recomendado es:
...
**Aclaración fundamental:** El supuesto de normalidad se refiere a los **errores** ($\varepsilon$), no a la variable dependiente ($Y$) ni a los predictores ($X$). Una $Y$ con distribución asimétrica (como costos hospitalarios o días de estadía) puede producir residuos perfectamente normales si el modelo está bien especificado. Verificar normalidad haciendo un histograma de $Y$ antes de ajustar el modelo es un error frecuente — lo que se evalúa son los **residuos del modelo ajustado**, no los datos crudos.
...
@razali2011 compararon la potencia de estos tests usando simulaciones con distribuciones alternativas simétricas (t de Student, logística, Cauchy) y asimétricas (chi-cuadrado, exponencial, log-normal, gamma), con tamaños de muestra entre 10 y 2000. La potencia depende fuertemente de *contra qué alternativa* se compara — un test puede ser muy potente contra colas pesadas pero débil contra asimetría, o viceversa. La tabla resume los patrones generales:
...
Shapiro-Wilk domina en muestras pequeñas y medianas contra la mayoría de alternativas. Jarque-Bera es especialmente potente contra alternativas con asimetría o curtosis anormal (frecuentes en datos de salud), pero pierde potencia con $n$ pequeño. Anderson-Darling es más sensible a desviaciones en las colas de la distribución.
...
::: {.callout-tip}
## Ejemplo clínico: estadía hospitalaria
Un caso clásico en datos de salud es modelar los **días de estadía hospitalaria** en función de variables como edad, diagnóstico o cantidad de procedimientos. Los pacientes con estadías cortas (1–3 días) tienen poca variabilidad entre sí: una apendicectomía sin complicaciones toma tiempos similares. Pero a medida que la estadía aumenta, la varianza explota: un paciente con 30 días de hospitalización puede tener desde una recuperación complicada pero predecible hasta una cadena de eventos adversos impredecibles. El resultado es un patrón de embudo (fan-shape) perfecto en el gráfico de residuos vs ajustados. En este contexto, usar los errores estándar OLS sin corrección puede llevar a declarar significativos predictores que no lo son — con implicancias directas en decisiones de gestión hospitalaria.

El mismo patrón aparece al modelar **tiempos de espera en listas quirúrgicas u oncológicas**. Si modelas días de espera en función del diagnóstico, la complejidad del procedimiento y la región del establecimiento, verás que los pacientes con esperas cortas (cirugías electivas simples) se agrupan con poca variabilidad, mientras que las esperas largas (pacientes oncológicos con protocolos de neoadyuvancia, pacientes que pierden turno por descompensación) generan una dispersión enorme. Un modelo que declara que "la región del hospital no afecta significativamente los tiempos de espera" puede estar usando errores estándar incorrectos que enmascaran diferencias reales entre la red urbana y la rural. La corrección con HC3 puede revertir esa conclusión.
:::
...
**¿Qué significa "HC"?** Son las siglas de *Heteroscedasticity-Consistent* (consistentes ante heterocedasticidad). La idea es simple: en vez de asumir que todos los errores tienen la misma varianza ($\sigma^2$), se estima una varianza diferente para cada observación a partir de los residuos. Existen varias formas de hacer esta estimación (HC0 a HC4), que difieren en cómo corrigen el sesgo en muestras finitas. **HC3 es la más recomendada** para muestras pequeñas y moderadas porque es la más conservadora — prefiere intervalos de confianza algo más anchos antes que errores estándar artificialmente pequeños.

Primero, veamos los resultados OLS estándar (que asumen varianza constante):

```{r}
#| label: errores-ols-estandar

summary(modelo)$coefficients
```

Ahora, los mismos coeficientes pero con errores estándar robustos HC3:
...
**¿Cómo interpretar la comparación?** Mira si los p-values cambian de forma sustantiva. Lo que importa no es si el número exacto cambia (siempre cambiará algo), sino si alguna variable **cruza el umbral de significancia**: era significativa con OLS estándar pero deja de serlo con HC3, o viceversa. Si eso ocurre, la heterocedasticidad está afectando tus conclusiones y la corrección no es opcional. Si los p-values se mueven pero las conclusiones son las mismas, el modelo es robusto a la heterocedasticidad y puedes reportar los resultados con confianza — preferiblemente con los errores HC3.
...
| **HC3** | **Más conservador** | **Recomendado para uso general** |
| HC4 | MacKinnon & White | Más robusto con puntos influyentes |

::: {.callout-tip}
## ¿Cuándo usar HC3 en la práctica?
Muchos investigadores en econometría y epidemiología usan HC3 **siempre**, independientemente de si Breusch-Pagan rechaza o no. La lógica es que los errores robustos son válidos tanto con homocedasticidad como sin ella (son ligeramente menos eficientes si la varianza es constante, pero nunca son incorrectos), mientras que los errores OLS estándar son válidos **solo** si la varianza es constante. En la duda, HC3 es la opción segura.
:::

**Referencias:** @zeileis2004; @long2000.

::: {.callout-note}
## Alternativa para heterocedasticidad severa: Weighted Least Squares (WLS)

Cuando la heterocedasticidad es severa y los errores robustos HC no bastan (los coeficientes cambian drásticamente al ponderar), se puede usar **mínimos cuadrados ponderados (WLS)**. La idea es dar menos peso a las observaciones con mayor varianza:

```r
# WLS: ponderando por el inverso de la varianza estimada
# Paso 1: estimar la varianza de los residuos como función de los fitted values
wts <- 1 / fitted(modelo)^2

# Paso 2: re-estimar el modelo con pesos
modelo_wls <- lm(mpg ~ weight + year + origin, data = datos, weights = wts)
summary(modelo_wls)
```

WLS es más eficiente que OLS + HC cuando la estructura de la varianza se conoce (o se estima bien). Sin embargo, requiere especificar *cómo* varía la varianza, lo que introduce una decisión adicional. En la práctica, la mayoría de los aplicados prefieren OLS + errores robustos HC3 como primera línea y reservan WLS para casos donde la estructura de varianza es conocida por teoría (e.g., varianza proporcional a un predictor). Para una cobertura completa, ver @wooldridge2020 [Cap. 8].
:::
...
::: {.callout-tip}
## Autocorrelación en datos de gestión hospitalaria

Si modelas **tiempos de atención en urgencia** a lo largo de los días del mes, la autocorrelación es casi inevitable: los días con alta demanda (lunes post-feriado, inicio de brotes respiratorios) generan acumulación que se arrastra a los días siguientes — si hoy colapsó la urgencia, mañana probablemente también. Los residuos del modelo estarán correlacionados porque el sistema de salud tiene **inercia**: la sobrecarga no se resuelve de un día para otro.

Lo mismo ocurre con **reportes epidemiológicos semanales** (casos de IRA, consultas GES): el conteo de esta semana predice el de la próxima porque los brotes tienen dinámica temporal. En estos contextos, el test de Breusch-Godfrey (con `bg_order` ajustado a la frecuencia de los datos) detectará autocorrelación real que debe abordarse con errores HAC o modelando explícitamente la estructura temporal. Un p-value sin corrección HAC podría hacerte creer que una intervención redujo los tiempos de espera cuando en realidad fue una fluctuación estacional.
:::

::: {.callout-warning}
## Contexto del dataset Auto antes de los tests

El dataset `Auto` está ordenado por año de fabricación (1970–1982), un período con cambios estructurales importantes: regulaciones CAFE de eficiencia, crisis del petróleo de 1973 y 1979, y cambios tecnológicos en motores. Esto significa que los tests de autocorrelación que siguen van a detectar dependencia *real* en el orden de los datos — no es un ejercicio ritual como sería en un corte transversal aleatorio. El predictor `year` captura la tendencia general, pero no los shocks discretos. Tenlo en cuenta al interpretar los resultados.
:::
...
**¿Qué significa `order`?** El parámetro `order` indica hasta cuántos **rezagos (lags)** hacia atrás buscar autocorrelación. Con `order = 1`, el test pregunta: "¿el residuo de la observación $i$ está correlacionado con el de la observación $i-1$?". Con `order = 7`, pregunta si hay correlación con alguno de los 7 residuos anteriores — útil para datos diarios con estacionalidad semanal (e.g., las admisiones hospitalarias de cada lunes se parecen entre sí). En general:

- `order = 1` — suficiente para detectar dependencia serial simple (AR(1)). Es el default.
- `order = 4` — datos trimestrales con posible patrón anual.
- `order = 7` — datos diarios con posible patrón semanal.
- `order = 12` — datos mensuales con posible estacionalidad anual.

Si tus datos no tienen orden temporal (encuestas, corte transversal), `order = 1` basta y el resultado debe interpretarse con cautela.
...
::: {.callout-note}
## Resultado: ¿autocorrelación o variables omitidas?

Como anticipamos, el DW bajo (~1.27) y el BG significativo confirman dependencia en los residuos. Pero como se advirtió antes de los tests, esto probablemente refleja **tendencias temporales no modeladas** (shocks petroleros, regulaciones CAFE), no autocorrelación serial clásica. La "autocorrelación" detectada es un síntoma de **variables omitidas con estructura temporal**. Posibles soluciones serían variables dummy para períodos regulatorios o splines en `year`, aunque para los fines de este ejercicio basta con reconocer esta limitación.
:::
...
**¿Cómo leer el gráfico ACF?** Cada barra vertical muestra la correlación entre los residuos y sus propios valores desplazados $k$ posiciones hacia atrás (lag $k$). Las bandas azules horizontales marcan el umbral de significancia: barras que las superan indican autocorrelación significativa en ese rezago. Los patrones a buscar son:

| Patrón en el ACF | Interpretación |
|---|---|
| Barra significativa solo en lag 1 | Autocorrelación serial simple (AR(1)) |
| Barras que decaen gradualmente | Dependencia persistente (común en series temporales) |
| Spike en lag 7 (datos diarios) | Estacionalidad semanal — e.g., admisiones hospitalarias |
| Spike en lag 12 (datos mensuales) | Estacionalidad anual — e.g., reportes epidemiológicos |
| Spikes en lags 7, 14, 21... | Estacionalidad semanal fuerte con armónicos |
| Todas las barras dentro de las bandas | Sin autocorrelación significativa |

::: {.callout-tip}
## Detectar estacionalidad en la práctica
Si trabajas con datos temporales (admisiones diarias, reportes mensuales, listas de espera), el ACF es tu primera herramienta para identificar patrones cíclicos. Un spike significativo en un lag específico te dice *a qué frecuencia* se repite el patrón, y eso determina el `order` que deberías usar en Breusch-Godfrey. Por ejemplo, si el ACF muestra un spike en lag 7, ejecuta `bgtest(modelo, order = 7)` para confirmar formalmente la estacionalidad semanal.
:::
...
::: {.callout-tip}
## Colinealidad natural en datos de salud

En datos clínicos, es común que las variables estén **naturalmente correlacionadas** porque miden aspectos distintos del mismo fenómeno biológico. Ejemplos frecuentes:

- **Peso, IMC y perímetro abdominal** — las tres miden composición corporal desde ángulos distintos.
- **Presión sistólica y diastólica** — ambas reflejan la función cardiovascular.
- **Creatinina y TFG estimada** — la segunda se calcula a partir de la primera.
- **Edad y número de comorbilidades** — correlacionadas porque las enfermedades crónicas se acumulan con el tiempo.

Si incluyes peso *e* IMC en el mismo modelo, el VIF explotará. Pero la solución **no es matemática** (centrar, escalar, aplicar ridge) — **es teórica**: elige la variable que tenga más sentido clínico para tu pregunta de investigación y elimina la redundante. Si investigas el efecto del sobrepeso en la estadía hospitalaria, el IMC es más interpretable que el peso bruto. Si investigas dosis de medicamento ajustada por tamaño corporal, el peso es la variable correcta.

La multicolinealidad entre variables clínicas no es un "problema a resolver" sino una señal de que estás incluyendo información redundante. El VIF te avisa; la decisión es tuya y debe basarse en la pregunta de investigación, no en el número.
:::
...
Para calcular el CN correctamente, @belsley1980 [Cap. 3] recomienda **centrar y escalar** las columnas de la matriz de diseño (excluyendo el intercepto). Si no se hace, la disparidad de magnitudes entre las variables puede inflar enormemente el CN sin que exista multicolinealidad real:

```{r}
#| label: condition-number-scaled

# CN con matriz centrada y escalada (sin intercepto) — el cálculo correcto
...
::: {.callout-warning}
## Cuidado con el CN sin escalar: artefacto de escala

Si calculas el CN sobre la matriz de diseño completa (con intercepto y sin escalar), el resultado puede ser engañosamente alto. Veamos qué pasa con nuestro modelo:
...
cat("Eigenvalores de X'X (sin escalar):\n")
print(round(valores_propios, 2))
cat(
  "\nCondition Number (sin escalar):",
  round(sqrt(max(valores_propios) / min(valores_propios)), 2),
  "\n"
)
```

El CN sin escalar da un valor > 1000, que normalmente indicaría colinealidad severa. Pero los VIF son todos bajos (< 5). ¿Qué ocurre? La columna de intercepto (todos 1s) convive con `weight` (valores ≈ 2000–5000), creando una disparidad de magnitudes que infla el ratio $\lambda_{\max}/\lambda_{\min}$ sin que exista dependencia lineal real. **El CN escalado (calculado arriba) es el diagnóstico correcto.** Siempre que VIF < 5 y CN sin escalar > 30, compare con el CN escalado antes de preocuparse.
:::
...
::: {.callout-tip}
## Antes de transformar: ¿el problema está en los datos o en el proceso?

En la práctica — especialmente en contextos clínicos y organizacionales — muchos outliers y puntos de alto leverage no reflejan fenómenos reales sino **problemas en el proceso de captura de datos**. Errores de digitación (un peso de 800 kg en vez de 80 kg), sistemas de información fragmentados que duplican o pierden registros, campos que cambian de unidad entre períodos (días vs. horas), o valores por defecto que el sistema asigna cuando un campo queda vacío (e.g., edad = 0 o fecha = 01/01/1900).

Estos errores se manifiestan en el diagnóstico como outliers o puntos de leverage extremo, y la tentación natural es aplicar una transformación logarítmica o eliminar la observación. Pero si la causa es un error de registro, la solución correcta no es estadística — es **auditar el proceso de captura de datos**. Antes de transformar o eliminar, pregúntate: ¿este valor es biológica o administrativamente plausible? Si la respuesta es no, la prioridad es corregir el dato en la fuente o excluirlo con una justificación documentada, no enmascarar el problema con una transformación.

Esta distinción es particularmente relevante en instituciones de salud, donde la calidad del dato depende de múltiples actores (admisión, enfermería, laboratorio, codificación) y la fragmentación de los sistemas de información es la norma, no la excepción.
:::

::: {.callout-tip}
## Outliers en datos agregados: hospitales con comportamiento anómalo

Cuando el análisis no es a nivel de paciente sino a nivel de **establecimiento** (e.g., modelar el promedio de días de estadía por hospital en función del volumen de cirugías, la dotación de camas y la complejidad del case-mix), los outliers adquieren un significado distinto: un hospital con residuo studentizado > 3 no es un "error de digitación" — es un **establecimiento que se comporta de forma anómala** respecto al patrón de la red.

Ese hospital puede tener una estadía promedio mucho mayor de lo esperado por su complejidad (¿problemas de gestión de altas? ¿infecciones intrahospitalarias?), o mucho menor (¿alta precoz inadecuada? ¿derivación de pacientes complejos?). En ambos casos, el outlier es **información valiosa para la gestión**, no ruido a eliminar. Antes de sacarlo del modelo, investiga qué lo hace diferente — ahí puede estar el hallazgo más importante del análisis.

Un punto de **alto leverage** en este contexto sería un hospital con valores inusuales en los predictores: un establecimiento rural muy pequeño con alta complejidad, o un hospital de referencia nacional sin comparables en la red. Estos puntos tienen el poder de "arrastrar" la recta hacia ellos, distorsionando los coeficientes para toda la red. La distancia de Cook te dice si ese efecto es sustantivo.
:::
...
### Investigar observaciones problemáticas con `augment()`

Los cálculos anteriores (Cook's D, leverage, residuos studentizados) identifican *cuáles* observaciones son problemáticas, pero para investigar *por qué* lo son, necesitas ver esas observaciones junto con sus variables originales. La función `augment()` del paquete `diagols` pega todos los diagnósticos al dataset original y agrega flags booleanos para filtrar directamente:

```{r}
#| label: augment-ejemplo
#| message: false
#| warning: false

# augment() requiere el paquete diagols instalado
library(diagols)
dx_aug <- diagols::diagnostico_ols(modelo)
datos_diagnostico <- augment(dx_aug, datos)

# Ver las observaciones más influyentes con sus variables originales
datos_diagnostico |>
  dplyr::filter(.cooksd_flag) |>
  dplyr::arrange(dplyr::desc(.cooksd)) |>
  dplyr::select(mpg, weight, year, origin, .cooksd, .hat, .std.resid) |>
  head(5)
```

Esto permite responder preguntas como: "¿Las observaciones influyentes son autos particularmente pesados? ¿De un año específico? ¿De un origen particular?" — en vez de mirar solo números de fila sin contexto.
...
diagnostico_ols <- function(mod, alpha = 0.05, bg_order = 1) {
  ...
  # Decisión basada en mayoría de tests disponibles
  norm_pvals <- c(
  ...
  cat(
    ...
    ifelse(
      norm_rechazos >= 2,
      "Mayoría rechaza H0 → Evidencia de no normalidad",
      ifelse(
        norm_rechazos == 1,
  ...
  cat(
    ...
    ifelse(
      bg$p.value < alpha,
      paste0("Rechaza H0 → Evidencia de autocorrelación (hasta orden ", bg_order, ")"),
  ...
  if (k < 2) {
    ...
    cat(
      ...
      ifelse(
        ...
        ifelse(
          max_vif > 5 & cn > 30,
          "VIF > 5 y CN > 30 → Multicolinealidad problemática",
          ifelse(
            max_vif > 5,
  ...
  # 6. Diagnóstico de influencia (Cook's D + leverage + studentized residuals)
...
**Cómo leer esta función:** `diagnostico_ols()` hace dos cosas. Primero, imprime un reporte completo en consola con el resultado de cada test y un semáforo resumen. Segundo, devuelve *invisiblemente* una lista de R con todos los objetos de los tests, para que puedas trabajar con ellos después. Para acceder a los datos tabulares, guarda el resultado en un objeto:

```r
resultados <- diagnostico_ols(modelo)
resultados$resumen           # dataframe con todos los tests
resultados$shapiro           # objeto del test de Shapiro-Wilk
resultados$flags             # vector lógico del semáforo
resultados$leverage          # hat values de cada observación
resultados$robust_se_cambio_pct  # cambio % en SE (OLS vs HC3)
```

La lista completa de elementos disponibles es: `resumen`, `shapiro`, `lilliefors`, `jarque_bera`, `breusch_pagan`, `reset`, `breusch_godfrey`, `vif`, `condition_number`, `cooks`, `leverage`, `studentized_residuals`, `robust_se_cambio_pct` y `flags`.

::: {.callout-tip}
## Instalar como paquete de R

Si quieres usar `diagnostico_ols()`, `plot_diagnostico_ols()`, `augment()` y `comparar_modelos()` en tus propios proyectos sin copiar el código, puedes instalarlas directamente como el paquete `diagols` desde el [repositorio de este curso](https://github.com/paulovillarroel/curso-regresion-lineal):
...
Las dependencias se instalan automáticamente. Después, en cualquier script o proyecto:

```r
library(diagols)

modelo <- lm(y ~ x1 + x2, data = mis_datos)
dx <- diagnostico_ols(modelo)             # diagnóstico completo
plot_diagnostico_ols(dx)                   # panel 2x2
augment(dx) |> dplyr::filter(.cooksd_flag) # observaciones influyentes
comparar_modelos(modelo1, modelo2)         # comparación con AIC corregido
```

Para documentación completa del paquete, changelog y guía de uso, ver el [repositorio en GitHub](https://github.com/paulovillarroel/curso-regresion-lineal).
:::
...
Cuando `diagnostico_ols()` muestra varios tests en rojo, puede ser difícil saber por dónde empezar. El siguiente diagrama resume la lógica de decisión:

```{mermaid}
%%| fig-cap: "Flujo de decisión para re-especificación de un modelo OLS."
flowchart TD
    A["Ejecutar diagnostico_ols()"] --> B{"¿Falla RESET?"}
    B -- Sí --> C["Revisar CR-plots"]
    C --> D{"¿Patrón parabólico?"}
    D -- Sí --> E["Término cuadrático\n(centrar variable)"]
    D -- No --> F{"¿Relación multiplicativa\no cola larga?"}
    F -- Sí --> G["Transformación log"]
    F -- No --> H["Revisar predictores\nomitidos"]
    B -- No --> I{"¿Falla Breusch-Pagan?"}
    I -- Sí --> J{"¿Patrón de abanico\nen residuos vs fitted?"}
    J -- Sí --> G
    J -- No --> K["Usar errores\nrobustos HC3"]
    I -- No --> L{"¿Alta influencia\n(Cook's D > 1)?"}
    L -- Sí --> M{"¿Dato plausible\nclínicamente?"}
    M -- No --> N["Auditar registro\nen la fuente"]
    M -- Sí --> O["Evaluar impacto:\n¿cambian conclusiones\nsin esa obs.?"]
    L -- No --> P{"¿Falla normalidad?"}
    P -- Sí --> Q{"¿n > 100?"}
    Q -- Sí --> R["TCL aplica.\nInferencia válida."]
    Q -- No --> S["Considerar\ntransformación\no bootstrap"]
    P -- No --> T["Modelo OK"]
...
Este diagrama cubre los problemas que requieren **re-especificación del modelo**. La independencia (Breusch-Godfrey) y la multicolinealidad (VIF) no aparecen porque generalmente no se resuelven transformando variables sino con errores HAC, errores agrupados, o selección de predictores — temas cubiertos en sus secciones respectivas.
...
#### Todo en una tabla: `comparar_modelos()`

Todo el análisis anterior — la comparación manual de flags, el cálculo del AIC crudo, la corrección Jacobiana — puede resumirse en una sola llamada con la función `comparar_modelos()` del paquete `diagols`. Esta función corre `diagnostico_ols()` en cada modelo, detecta automáticamente cuáles tienen `log(Y)`, aplica la corrección Jacobiana y presenta todo en un tibble:

```{r}
#| label: comparar-modelos-diagols
#| message: false
#| warning: false

diagols::comparar_modelos(modelo, modelo_quad, modelo_log)
```

La columna `delta_AIC` identifica el mejor modelo (delta = 0), las columnas de flags muestran qué supuestos falla cada uno, y `n_problemas` resume cuántos problemas tiene. En una sola línea obtienes la misma información que construimos paso a paso en las secciones anteriores.
...
- **Independencia**: el test de Breusch-Godfrey detecta dependencia en el *orden* de los datos. El dataset `Auto` está ordenado por año de fabricación (1970–1982), por lo que esta dependencia probablemente refleja **tendencias temporales residuales** no capturadas por el predictor lineal `year` (shocks petroleros, regulaciones CAFE), no autocorrelación serial clásica (ver el callout en la @sec-independencia). Con el modelo log-log, la dependencia residual se reduce pero no desaparece. Si hubiera preocupación por clustering (e.g., autos del mismo fabricante), la solución sería errores agrupados, no una re-especificación.
...
::: {.callout-tip}
## Framework de trabajo con `diagols`

Este análisis sigue un flujo estándar que puedes replicar con cualquier modelo usando las funciones del paquete `diagols`:
...
# 2. Diagnóstico completo (imprime semáforo + retorna objeto)
dx <- diagnostico_ols(modelo)

# 3. Visualizar los 4 gráficos diagnósticos
plot_diagnostico_ols(dx)

# 4. Investigar observaciones problemáticas
augment(dx, datos) |> dplyr::filter(.cooksd_flag | .outlier_flag)

# 5. Si hay problemas de especificación, probar correcciones
modelo_v2 <- lm(log(y) ~ log(x1) + x2, data = datos)

# 6. Comparar modelos (AIC corregido + flags lado a lado)
comparar_modelos(modelo, modelo_v2)
```

Las funciones no reemplazan el análisis — **lo estructuran**. El juicio sobre qué corrección aplicar, si un outlier es plausible, o si la heterocedasticidad es severa, sigue siendo tuyo.
:::

> *En regresión, la mayoría de los problemas no se corrigen con tests — se corrigen con mejores modelos.*

### Guía rápida: ¿Cómo elegir la especificación en contextos de salud?

La elección de la forma funcional (lineal, cuadrática o logarítmica) no es un ejercicio de ensayo y error para forzar un p-value. Es una decisión teórica respaldada por evidencia empírica. En contextos clínicos y de gestión sanitaria, esta guía de tres pasos ayuda a tomar esa decisión de forma estructurada.

#### Paso 1: La naturaleza del fenómeno

Antes de ejecutar `lm()`, pregúntate cómo funciona el proceso en la realidad clínica o administrativa. Cada forma funcional captura un tipo distinto de relación entre las variables. La siguiente tabla resume las cinco especificaciones más comunes:

| Especificación | Fórmula | Interpretación de $\beta_1$ |
|---|---|---|
| **Lineal** (level-level) | $Y = \beta_0 + \beta_1 X$ | +1 unidad en $X$ → $+\beta_1$ unidades en $Y$ |
| **Log-lin** (log-level) | $\log Y = \beta_0 + \beta_1 X$ | +1 unidad en $X$ → $+\beta_1 \times 100$% en $Y$ |
| **Lin-log** (level-log) | $Y = \beta_0 + \beta_1 \log X$ | +1% en $X$ → $+\beta_1 / 100$ unidades en $Y$ |
| **Log-log** | $\log Y = \beta_0 + \beta_1 \log X$ | +1% en $X$ → $+\beta_1$% en $Y$ (elasticidad) |
| **Cuadrática** | $Y = \beta_0 + \beta_1 X + \beta_2 X^2$ | Efecto marginal = $\beta_1 + 2\beta_2 X$ (varía según $X$) |
| **Interacción** | $Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \beta_3 X_1 X_2$ | Efecto de $X_1$ = $\beta_1 + \beta_3 X_2$ (depende de $X_2$) |

A continuación, cada especificación se desarrolla con su explicación, cuándo usarla en contextos de salud, y cómo implementarla en R.

##### Lineal (level-level)

$$Y = \beta_0 + \beta_1 X$$

El modelo más simple: asume que el efecto de $X$ sobre $Y$ es **aditivo y constante**. Cada unidad adicional de $X$ suma la misma cantidad a $Y$, sin importar el nivel de $X$. Es el punto de partida natural y muchas veces suficiente.

**Cuándo usarlo en salud:** Cuando el efecto es razonablemente proporcional. Por ejemplo, si cada examen de laboratorio adicional agrega un costo fijo al total de la atención, independientemente de cuántos exámenes ya se hayan pedido.

```r
# Lineal: Y ~ X
modelo_lineal <- lm(costo ~ n_examenes + edad + sexo, data = datos)
summary(modelo_lineal)
# Interpretación: "Cada examen adicional se asocia con un aumento
# de beta_1 pesos en el costo total"
```
...
Se transforma solo la variable dependiente. Asume que el efecto de $X$ es **porcentual sobre $Y$**: cada unidad adicional de $X$ cambia $Y$ en un porcentaje constante, no en una cantidad fija. Esto produce un crecimiento (o decaimiento) exponencial.

**Cuándo usarlo en salud:** Cuando $Y$ tiene asimetría positiva fuerte (cola larga a la derecha) y el efecto de $X$ es multiplicativo. Ejemplo clásico: el decaimiento del volumen de pacientes en lista de espera según los días transcurridos. Cada día adicional no saca un número fijo de pacientes, sino que reduce el volumen en un porcentaje constante.

```r
# Log-lin: log(Y) ~ X
modelo_loglin <- lm(log(dias_estadia) ~ n_comorbilidades + edad + sexo,
                    data = datos)
summary(modelo_loglin)
# Interpretación: "Cada comorbilidad adicional se asocia con un aumento
# de (exp(beta_1) - 1) * 100 % en los días de estadía"
# Aproximación rápida si beta_1 es pequeño: beta_1 * 100 %
```
...
Se transforma solo el predictor. Captura **rendimientos decrecientes**: los primeros incrementos de $X$ tienen un efecto grande sobre $Y$, pero a medida que $X$ crece, el efecto marginal se atenúa. La curva sube rápido al principio y luego se aplana.

**Cuándo usarlo en salud:** Cuando el recurso tiene un rendimiento que se satura. Ejemplo: el efecto del gasto per cápita en salud sobre la esperanza de vida. Pasar de \$100 a \$200 per cápita tiene un impacto enorme; pasar de \$5.000 a \$5.100 casi no cambia nada. La transformación logarítmica del gasto captura esta asimetría.

```r
# Lin-log: Y ~ log(X)
modelo_linlog <- lm(esperanza_vida ~ log(gasto_percapita) + educacion,
                    data = datos)
summary(modelo_linlog)
# Interpretación: "Duplicar el gasto per cápita (aumento de 100%)
# se asocia con un aumento de beta_1 * log(2) ≈ beta_1 * 0.693
...
Se transforman ambas variables. El coeficiente $\beta_1$ es directamente una **elasticidad**: un cambio de 1% en $X$ se asocia con un cambio de $\beta_1$% en $Y$. Esta es la especificación natural cuando la relación entre las variables es multiplicativa en ambos sentidos.

**Cuándo usarlo en salud:** Cuando tanto $Y$ como $X$ tienen distribuciones asimétricas y el efecto es proporcional. Ejemplo: la relación entre estadía hospitalaria y peso de los pacientes. Un kilo extra no impacta igual a un paciente de 50 kg que a uno de 120 kg — el efecto es proporcional al nivel base. Además, la doble transformación logarítmica suele estabilizar la varianza (resolviendo heterocedasticidad) y comprimir valores extremos (reduciendo el impacto de outliers).

```r
# Log-log: log(Y) ~ log(X)
modelo_loglog <- lm(log(dias_estadia) ~ log(peso) + edad + sexo,
                    data = datos)
summary(modelo_loglog)
# Interpretación: "Un aumento del 10% en el peso se asocia con un
# cambio de beta_1 * 10 % en los días de estadía"
```
...
Permite que el efecto de $X$ **cambie de dirección o se atenúe**. A diferencia de las transformaciones logarítmicas, la cuadrática puede capturar relaciones en U o en U invertida. El efecto marginal no es constante: depende del valor de $X$ según $\frac{\partial Y}{\partial X} = \beta_1 + 2\beta_2 X$.

**Cuándo usarlo en salud:** Cuando hay un punto de inflexión o un óptimo. Ejemplo clásico: la mortalidad posquirúrgica según edad. Es alta en neonatos, baja en adultos jóvenes y vuelve a subir en la tercera edad. Un modelo lineal diría que "la edad no afecta" (promediando subidas y bajadas) o que el efecto es monótono; el cuadrático captura la curva en U.

**Importante:** centrar la variable antes de elevarla al cuadrado para evitar multicolinealidad artificial entre $X$ y $X^2$ (ver la sección de re-especificación de este documento).

```r
# Cuadrática: Y ~ X + X^2 (centrado)
datos$edad_c <- datos$edad - mean(datos$edad)
modelo_cuad <- lm(mortalidad ~ edad_c + I(edad_c^2) + sexo + asa_score,
                  data = datos)
summary(modelo_cuad)
# Interpretación: "El efecto de la edad sobre la mortalidad no es
# constante. En la edad promedio, el efecto marginal es beta_1.
# Si beta_2 > 0, la mortalidad aumenta más rápido en los extremos."
```

##### Interacciones ($X_1 \times X_2$): efectos modificadores

$$Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \beta_3 (X_1 \times X_2) + \varepsilon$$

Todas las especificaciones anteriores asumen que el efecto de cada variable es **independiente** del valor de las demás. Pero en la realidad clínica, el efecto de una variable frecuentemente *depende del contexto* del paciente. Esto se llama **efecto modificador** o **interacción**: el efecto de $X_1$ sobre $Y$ cambia según el nivel de $X_2$.

El coeficiente de interacción $\beta_3$ mide **cuánto cambia el efecto de $X_1$ cuando $X_2$ aumenta en una unidad** (o cuando $X_2$ pasa de 0 a 1, si es binaria). El efecto marginal de $X_1$ ya no es constante: es $\beta_1 + \beta_3 \cdot X_2$.

**Cuándo usarlo en salud:** Cuando sospechas que el efecto de un tratamiento o exposición no es igual para todos los pacientes. Ejemplo: un protocolo de rehabilitación podría no mostrar efecto *en promedio* sobre los días de estadía, pero ser altamente efectivo en pacientes con una comorbilidad específica (diabetes, por ejemplo). Sin la interacción, el modelo promedia el efecto y lo declara no significativo — enmascarando un hallazgo clínicamente relevante.

```r
# Interacción: el efecto de tratamiento depende de la comorbilidad
modelo_inter <- lm(dias_estadia ~ tratamiento * diabetes + edad + sexo,
                   data = datos)
summary(modelo_inter)
# R expande tratamiento * diabetes en:
#   tratamiento + diabetes + tratamiento:diabetes
#
# Interpretación:
#   beta_tratamiento = efecto del tratamiento cuando diabetes = 0
#   beta_diabetes = efecto de diabetes cuando tratamiento = 0
#   beta_tratamiento:diabetes = cuánto CAMBIA el efecto del tratamiento
#                               si el paciente tiene diabetes
```

::: {.callout-tip}
## ¿Cómo comunicar una interacción?
En vez de: *"El tratamiento reduce la estadía en 2 días"*, se reporta: *"El tratamiento no muestra efecto significativo en pacientes sin diabetes ($\beta = -0.3$, p = 0.42), pero reduce la estadía en 4.1 días en pacientes con diabetes ($\beta_{\text{trat}} + \beta_{\text{trat:diab}} = -0.3 + (-3.8) = -4.1$, p < 0.01)"*. Para el clínico, esto es mucho más útil que un promedio general.
:::

#### Paso 2: ¿Qué dice `diagnostico_ols()`?

Si la teoría no es concluyente, el análisis de residuos guía el ajuste. Estos son los patrones más comunes y qué hacer en cada caso:

- **Falla RESET (Ramsey):** Es la señal principal de mala especificación. Revisa el gráfico de residuos vs ajustados: si ves una "U" o un patrón curvo, prueba un término cuadrático centrado. Si la curva se parece más a una exponencial que a una parábola, prueba la transformación logarítmica.

- **Falla Breusch-Pagan con patrón de abanico:** Un modelo que se "desparrama" en valores altos a menudo esconde una relación multiplicativa. Prueba `log(Y)`. Si la varianza se estabiliza y el QQ-plot mejora simultáneamente, la forma funcional correcta era multiplicativa — y la heterocedasticidad era un síntoma, no el problema real.

- **Cook's D con observaciones extremas arrastrando la recta:** Muy común en costos de salud o estadías prolongadas. Antes de eliminar esas observaciones, prueba una transformación `log()`: comprime naturalmente los extremos, reduciéndoles leverage sin tener que sacarlos del registro. Si después de transformar el modelo los diagnósticos de influencia mejoran, el problema no eran las observaciones sino la escala del modelo.

#### Paso 3: La interpretabilidad para el usuario final

El mejor modelo del mundo no sirve si no puedes explicárselo a un referente técnico o a un director de hospital:

- **Lineal:** *"Un año extra de edad se asocia con 1.5 días más de estadía."*
- **Log-lin:** *"Cada comorbilidad adicional aumenta la estadía en un 15%."*
- **Lin-log:** *"Duplicar el gasto per cápita se asocia con 2.3 años más de esperanza de vida."*
- **Log-log:** *"Un aumento del 10% en el presupuesto reduce los tiempos de espera en un 5%."*
- **Cuadrática:** *"El efecto de la edad cambia según qué tan mayor sea el paciente — es mayor en los extremos."*
- **Interacción:** *"El tratamiento no muestra efecto en pacientes sin diabetes, pero reduce la estadía en 4 días en pacientes con diabetes."*

::: {.callout-important}
## Regla de oro
Si una transformación compleja (como un polinomio de grado 3) mejora marginalmente el ajuste pero destruye la capacidad de comunicar el hallazgo de forma sencilla, quédate con el modelo más simple y reporta las limitaciones. Un modelo lineal con errores robustos y una interpretación clara puede ser más útil para la toma de decisiones que un modelo perfecto que nadie entiende.
:::

::: {.callout-tip}
## Caso de uso: La "cola larga" de las listas de espera

Uno de los problemas más comunes en gestión sanitaria es modelar los tiempos de las listas de espera (quirúrgicas u oncológicas). Si graficamos el **volumen de pacientes** ($Y$) versus los **días de espera** ($X$), casi siempre observamos un comportamiento extremo: una masa de pacientes que ingresó recientemente (días bajos), seguida de una caída abrupta y una "cola larga" de pacientes que se arrastra por meses o años.

**¿Qué pasa si usamos OLS lineal?** La recta intentará pasar por el medio del volumen masivo inicial, caerá en picada y cruzará el eje $X$, terminando por predecir una cantidad *negativa* de pacientes para los tiempos de espera más largos. Un modelo lineal promedia el fenómeno y oculta a los pacientes de mayor riesgo clínico.

**¿Cómo se especifica correctamente?**

1. **Log-lin** ($\log(Y) = \beta_0 + \beta_1 X$) — Asume decaimiento exponencial. El volumen no cae en un número fijo de pacientes por día, sino en un *porcentaje* constante.
2. **Log-log** ($\log(Y) = \beta_0 + \beta_1 \log(X)$) — Comprime tanto el pico inicial como la cola, estabilizando la varianza si el decaimiento es más lento que exponencial.
3. **Más allá de OLS:** Dado que el volumen de pacientes es una variable de conteo (no hay 1.5 pacientes), en un entorno avanzado se usaría una regresión de Poisson o **binomial negativa** (GLM). Esta especificación respeta la naturaleza de los números enteros, nunca predice valores negativos y maneja la sobredispersión inherente a estos datos.
:::

::: {.callout-note}
## Más allá de OLS: ¿Cuándo considerar otros modelos?

Este curso se enfoca en la regresión lineal clásica (OLS), que es la base de la modelación estadística. Pero en contextos de salud, muchas variables de resultado no se ajustan bien a un modelo lineal, sin importar cuántas transformaciones apliques:

- **Variables de conteo** (número de consultas, reingresos, eventos adversos) → Regresión de Poisson o Binomial Negativa.
- **Variables binarias** (mortalidad sí/no, readmisión sí/no) → Regresión Logística.
- **Tiempos hasta un evento** (sobrevida, tiempo hasta el alta) → Modelos de supervivencia (Cox).

Si después de diagnosticar y re-especificar tu modelo OLS sigues con problemas persistentes (residuos que no pueden ser normales, predicciones negativas para una variable que solo puede ser positiva), probablemente la solución no es otra transformación sino **otro tipo de modelo**. Los modelos lineales generalizados (GLM) extienden la lógica de OLS a estas situaciones, manteniendo la misma estructura de coeficientes e interpretación que ya aprendiste aquí.

Si además tu objetivo es estimar **efectos causales** (no solo asociaciones), el siguiente paso es combinar estos modelos con técnicas de identificación causal: propensity scores, G-computation, métodos doblemente robustos o variables instrumentales. Para una guía completa implementada en R, ver @rcausal2024 [Cap. 10–21] y @huntingtonklein2021 [Cap. 13–20].
:::
