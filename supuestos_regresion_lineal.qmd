...
| Especificación | Cuándo usarla | Ejemplo en salud |
|---|---|---|
| **Lineal** ($Y = \beta_0 + \beta_1 X$) | El efecto es **aditivo y constante**. Cada unidad adicional de $X$ suma la misma cantidad a $Y$, sin importar el nivel de $X$. | Por cada examen de laboratorio extra que se solicita, el costo total de la atención sube en una cantidad fija, independientemente de si el paciente lleva 1 o 10 exámenes previos. |
| **Cuadrática** ($Y = \beta_0 + \beta_1 X + \beta_2 X^2$) | Existe un efecto de **saturación, rendimientos decrecientes o un patrón en U**. El efecto de $X$ cambia de dirección o se atenúa. | La mortalidad posquirúrgica según edad: alta en neonatos, baja en adultos jóvenes, vuelve a subir en la tercera edad. Un modelo lineal diría erróneamente que "la edad no afecta" o que el efecto es monótono; el cuadrático captura la curva. |
| **Log-log** ($\log Y = \beta_0 + \beta_1 \log X$) | El fenómeno es **multiplicativo**: un cambio porcentual en $X$ produce un cambio porcentual constante en $Y$. La variable dependiente tiene asimetría positiva (cola larga). | Tiempos de estadía hospitalaria o días en lista de espera oncológica. Una comorbilidad grave no suma "2 días fijos" a la espera, sino que probablemente *multiplica* el tiempo base en un porcentaje. |
