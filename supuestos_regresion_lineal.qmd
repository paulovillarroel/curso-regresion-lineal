...
Antes de ejecutar `lm()`, pregúntate cómo funciona el proceso en la realidad clínica o administrativa:

| Especificación | Fórmula | Interpretación de $\beta_1$ | Ejemplo en salud |
|---|---|---|---|
| **Lineal** (level-level) | $Y = \beta_0 + \beta_1 X$ | Un aumento de 1 unidad en $X$ se asocia con un cambio de $\beta_1$ unidades en $Y$. | Por cada examen de laboratorio extra, el costo sube en una cantidad fija, independientemente de si el paciente lleva 1 o 10 exámenes previos. |
| **Log-lin** (log-level) | $\log Y = \beta_0 + \beta_1 X$ | Un aumento de 1 unidad en $X$ se asocia con un cambio de $\beta_1 \times 100$% en $Y$. | Decaimiento del volumen de pacientes en lista de espera: cada día adicional reduce el volumen en un porcentaje constante (decaimiento exponencial). |
| **Lin-log** (level-log) | $Y = \beta_0 + \beta_1 \log X$ | Un aumento de 1% en $X$ se asocia con un cambio de $\beta_1 / 100$ unidades en $Y$. | Efecto del gasto per cápita en salud sobre la esperanza de vida: duplicar el gasto mejora la esperanza de vida, pero cada peso adicional rinde menos que el anterior (rendimientos decrecientes). |
| **Log-log** | $\log Y = \beta_0 + \beta_1 \log X$ | Un aumento de 1% en $X$ se asocia con un cambio de $\beta_1$% en $Y$ (elasticidad). | Tiempos de estadía hospitalaria. Una comorbilidad grave no suma "2 días fijos" a la espera, sino que *multiplica* el tiempo base en un porcentaje. |
| **Cuadrática** | $Y = \beta_0 + \beta_1 X + \beta_2 X^2$ | El efecto de $X$ no es constante: depende del nivel de $X$. Efecto marginal = $\beta_1 + 2\beta_2 X$. | Mortalidad posquirúrgica según edad: alta en neonatos, baja en adultos jóvenes, vuelve a subir en la tercera edad. Un modelo lineal no capturaría esta curva en U. |
...
- **Coeficientes lineales** se comunican directo: *"Un año extra de edad se asocia con 1.5 días más de estadía"*.
- **Coeficientes cuadráticos** requieren explicar que el efecto no es constante: *"El efecto de la edad cambia dependiendo de qué tan mayor sea el paciente — es mayor en los extremos"*.
- **Coeficientes log-log** se leen como elasticidades (porcentajes): *"Un aumento del 10% en el presupuesto del servicio se asocia con una reducción del X% en los tiempos de espera"*.
